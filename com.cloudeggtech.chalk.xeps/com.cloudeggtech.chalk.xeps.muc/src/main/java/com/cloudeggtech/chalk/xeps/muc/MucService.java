...
import com.cloudeggtech.basalt.xeps.muc.user.Continue;
import com.cloudeggtech.basalt.xeps.muc.user.Invite;
import com.cloudeggtech.basalt.xeps.muc.user.MucUser;
import com.cloudeggtech.basalt.xeps.muc.user.Status;
...
import com.cloudeggtech.chalk.xeps.muc.events.ChangeAvailabilityStatus;
import com.cloudeggtech.chalk.xeps.muc.events.ChangeAvailabilityStatusEvent;
import com.cloudeggtech.chalk.xeps.muc.events.ChangeNick;
import com.cloudeggtech.chalk.xeps.muc.events.ChangeNickEvent;
import com.cloudeggtech.chalk.xeps.muc.events.DiscussionHistoryEvent;
import com.cloudeggtech.chalk.xeps.muc.events.Enter;
import com.cloudeggtech.chalk.xeps.muc.events.EnterEvent;
import com.cloudeggtech.chalk.xeps.muc.events.Exit;
import com.cloudeggtech.chalk.xeps.muc.events.ExitEvent;
import com.cloudeggtech.chalk.xeps.muc.events.Invitation;
import com.cloudeggtech.chalk.xeps.muc.events.InvitationEvent;
import com.cloudeggtech.chalk.xeps.muc.events.Kick;
import com.cloudeggtech.chalk.xeps.muc.events.KickEvent;
import com.cloudeggtech.chalk.xeps.muc.events.Kicked;
import com.cloudeggtech.chalk.xeps.muc.events.KickedEvent;
import com.cloudeggtech.chalk.xeps.muc.events.PrivateMessageEvent;
import com.cloudeggtech.chalk.xeps.muc.events.RoomEvent;
import com.cloudeggtech.chalk.xeps.muc.events.RoomMessage;
import com.cloudeggtech.chalk.xeps.muc.events.RoomMessageEvent;
import com.cloudeggtech.chalk.xeps.muc.events.RoomSubject;
import com.cloudeggtech.chalk.xeps.muc.events.RoomSubjectEvent;
...
public class MucService implements IMucService, IMessageListener, IPresenceListener {
	...
	@Override
	public int getTotalNumberOfRooms(final JabberId host) throws ErrorException {
		if (host.getName() != null || host.getResource() != null) {
	...
	@Override
	public JabberId[] getMucRooms() {
		try {
			return doGetMucRooms();
		} catch (ErrorException e) {
			throw new RuntimeException("Unexpected error.", e);
		}
	}

	private JabberId[] doGetMucRooms() throws ErrorException {
		return chatServices.getTaskService().execute(new ISyncTask<Iq, JabberId[]>() {

			@Override
			public void trigger(IUnidirectionalStream<Iq> stream) {
				DiscoItems discoItems = new DiscoItems();
				discoItems.setNode("http://jabber.org/protocol/muc#rooms");
				
				Iq iq = new Iq();				
				iq.setTo(chatServices.getStream().getJid());
				iq.setObject(discoItems);
			...
			public JabberId[] processResult(Iq iq) {
				...
				JabberId[] mucRooms = new JabberId[discoItems.getItems().size()];
				for (int i = 0; i< discoItems.getItems().size(); i++) {
					Item item = discoItems.getItems().get(i);
					JabberId jid = new JabberId(item.getJid().getName(), item.getJid().getDomain(), item.getName());
					mucRooms[i] = jid;
				}
	...
	public void received(Presence presence) {
		if (presence.getObject() instanceof MucUser) {
			...
			RoomEvent<?> event;
			com.cloudeggtech.basalt.xeps.muc.user.Item item = mucUser.getItems().get(0);

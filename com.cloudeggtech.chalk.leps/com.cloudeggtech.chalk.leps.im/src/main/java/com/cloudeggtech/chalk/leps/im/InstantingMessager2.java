...
import com.cloudeggtech.chalk.im.roster.RosterService;
...
import com.cloudeggtech.chalk.leps.im.subscription.LepSubscriptionService;
import com.cloudeggtech.chalk.leps.im.subscription.StandardSubscriptionService;
...
public class InstantingMessager2 implements IInstantingMessager2,
	...
	@Override
	public void sendMessageReadAck(JabberId contact, String messageId) {
		Iq ack = new Iq(Iq.Type.SET, Stanza.generateId(TRACE_ID_PREFIX));
		ack.setTo(contact);
		
		ack.setObject(new Trace(new MsgStatus(messageId, MsgStatus.Status.MESSAGE_READ,
				getJid(), new DateTime())));
